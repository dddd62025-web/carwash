...
import { getServices, createJob, Service, Job } from '@/lib/db';
...
export default function ServiceChecklist({
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      ...
      const savedJob = await createJob(employeeId, selectedBrand, totalAmount, selectedServicesArray);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center p-4 min-h-[150px] text-gray-800">
        <Loader2 className="w-8 h-8 text-blue-650 animate-spin mb-2" />
        <p className="text-gray-400 font-semibold text-xs">Chargement des prestations...</p>
      </div>
    );
  }
  ...
  return (
    <div className={`w-full flex flex-col text-gray-800 ${
      isCompact 
        ? 'bg-transparent pb-4' 
        : 'max-w-4xl mx-auto bg-gray-50 min-h-screen pb-40 px-3 sm:px-6'
    }`}>
      ...
      {/* Main Content */}
      <main className={`${isCompact ? 'py-2' : 'py-6'} flex-1`}>
        {errorMessage && (
        ...
        {/* Selected vehicle summary in compact mode */}
        {isCompact && (
          <div className="flex items-center justify-between bg-white border border-gray-200 p-3 rounded-2xl mb-4 shadow-sm">
            <div className="min-w-0 flex-1">
              <p className="text-[10px] text-gray-400 font-bold uppercase">Prestations du véhicule</p>
              <h4 className="text-sm font-black text-gray-900 truncate uppercase mt-0.5">{selectedBrand}</h4>
            </div>
            <button
              onClick={onCancel}
              className="text-xs font-bold text-red-500 bg-red-50 hover:bg-red-100 py-1.5 px-3 rounded-xl transition-colors shrink-0"
            >
              Changer
            </button>
          </div>
        ...
        <div className={
          isCompact 
            ? "grid grid-cols-1 gap-2.5 max-h-72 overflow-y-auto pr-1"
          ...
          {services.map((service) => {
            ...
            return (
              <button
                ...
                className={`rounded-2xl text-left flex items-start justify-between transition-all duration-200 active:scale-[0.98] ${
                  isCompact 
                    ? 'p-3.5 min-h-[75px]' 
                    : 'p-5 sm:p-6 min-h-[110px]'
                } ${
                  isSelected
              ...
              >
                <div className="flex-1 pr-2 min-w-0">
                  ...
                  <p className={`font-black text-blue-600 ${
                    isCompact ? 'text-base mt-1' : 'text-lg sm:text-xl mt-2'
                  }`}>
                    {service.price.toFixed(2)} <span className="text-[10px] font-bold text-blue-500">DT</span>
                  </p>
                </div>
                ...
                >
                  {isSelected && <Check className={isCompact ? "w-3 h-3 stroke-[3]" : "w-3.5 h-3.5 stroke-[3]"} />}
                </div>
              </button>
            );
          })}
        ...
        {/* Compact mode inline footer summary */}
        {isCompact && (
          <div className="bg-white border border-gray-200 p-4 rounded-2xl mt-4 shadow-sm space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-gray-400 uppercase">Montant Total :</span>
              <span className="text-xl font-black text-blue-600">
                {calculateTotal().toFixed(2)} <span className="text-xs font-bold">DT</span>
              </span>
            </div>
            <button
              onClick={() => setIsModalOpen(true)}
              disabled={selectedServices.length === 0}
              className="w-full py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-200 disabled:text-gray-400 disabled:cursor-not-allowed text-white font-extrabold text-sm rounded-xl transition-all shadow-md active:scale-95 flex items-center justify-center space-x-2"
            >
              <ClipboardList className="w-4 h-4" />
              <span>Enregistrer le lavage</span>
            </button>
          </div>
        )}
      </main>

      {/* Floating Footer (Hidden in compact mode) */}
      {!isCompact && (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-3">
          <div className="w-full max-w-md bg-white rounded-3xl overflow-hidden shadow-2xl border border-gray-100 animate-in fade-in zoom-in-95 duration-150 text-left">
            {/* Modal Header */}
            ...
            {!successMessage && (
              <div className="bg-gray-50 px-5 py-4 border-t border-gray-100 flex space-x-3">
                <button
                  onClick={() => setIsModalOpen(false)}
                  disabled={submitting}
                  className="flex-1 py-3 bg-white border border-gray-250 hover:bg-gray-100 text-gray-700 font-bold rounded-xl text-xs sm:text-sm transition-colors disabled:opacity-50 active:scale-95 text-center"
                >
                  Annuler
                </button>
                <button
